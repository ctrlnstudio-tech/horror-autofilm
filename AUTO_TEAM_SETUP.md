...
ระบบตั้งค่า worker ปกติใน `auto_config.json` ให้สร้างและอัปโหลดวิดีโอ SHORT เวลา `18:00` ตามเวลาไทย
...
ค่าเริ่มต้นคือสร้างคลิป `short` รอบละ 1 คลิป เวลา `18:00` ตามเวลาไทย แล้วอัปโหลด YouTube เป็น `public` พร้อมระบุว่าเป็น AI video
...
GitHub Actions จะรันสองงานต่อวันตามเวลาไทย:

- `17:00` สร้างวิดีโอ `long` แนวนอน ความยาวไม่น้อยกว่า 10 นาที
- `18:00` สร้างวิดีโอ `short` แนวตั้ง

กด `Run workflow` เพื่อทดสอบเองได้ โดยเลือก `short` หรือ `long` ก่อนเริ่มงาน
